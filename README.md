...
Optionally, generate Tenstorrent kernel code for matmul:

```bash
TILELOOM_TO_TTKERNEL_OPTIONS="matmul-stream-output-subblocks matmul-merge-b-reader-into-writer matmul-split-half-dm-cores" ./third_party/loom2ttkernel/lower.sh test/matmul_2Dmesh/IRs/p03_bufferized.mlir 1
```

Profile performance, you may change shape M16384_N16384_K16384 to what you use:
```bash
python3 tests/host_matmul.py M16384_N16384_K16384  --perf_target both --iters 100
```

or for other kernels:
```bash
./third_party/loom2ttkernel/lower.sh test/xxx_2Dmesh/IRs/p03_bufferized.mlir 1
```
