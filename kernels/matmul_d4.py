...
def _matmul_d4(x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
    ...
    for tile_m, tile_n in hl.tile([m, n]):
        ...
        out_[tile_m, tile_n] = torch.log(torch.exp(torch.log(torch.exp(acc))))
    return out_


class MatmulD4(LoomKernel):
